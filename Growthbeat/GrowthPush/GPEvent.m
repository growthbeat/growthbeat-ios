...
+ (GPEvent *)createWithGrowthbeatClient:(NSString *)clientId applicationId:(NSString *)applicationId credentialId:(NSString *)credentialId type:(GPEventType)eventType name:(NSString *)name value:(NSString *)value {
    ...
    if (!httpResponse.success) {
        [[[GrowthPush sharedInstance] logger] error:@"Failed to create event. %@", httpResponse.error ? httpResponse.error : [NSString stringWithFormat:@"code:%@, %@", [httpResponse.body objectForKey:@"code"], [httpResponse.body objectForKey:@"message"]]];
        return nil;
    }
